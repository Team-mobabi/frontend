...
export default function ResetConfirmModal({ open, onClose, onConfirm, commitHash, commitMessage }) {
    ...
    const handleResetClick = (mode) => {
        const warnings = {
            soft: {
                title: "Soft Reset",
                message: `현재 가지를 이 저장(${commitHash.slice(0, 7)})으로 되돌리시겠습니까?\n\n` +
                         `✅ 저장 기록만 취소됩니다\n` +
                         `✅ 변경된 파일은 "담긴 상태"로 유지됩니다\n` +
                         `✅ 파일 내용은 그대로 남아있습니다\n\n` +
                         `💡 저장 메시지를 수정하거나 다시 저장할 때 유용합니다.`
            },
            hard: {
                title: "Hard Reset",
                message: `⚠️ 경고: 현재 가지를 이 저장(${commitHash.slice(0, 7)})으로 되돌리시겠습니까?\n\n` +
                         `❌ 저장 기록이 취소됩니다\n` +
                         `❌ 변경된 파일도 모두 삭제됩니다\n` +
                         `❌ 작업 디렉토리가 해당 저장 시점으로 완전히 되돌아갑니다\n\n` +
                         `🚨 이 작업은 되돌릴 수 없습니다!`
            }
        };
    ...
    const modeDescriptions = {
        soft: {
            title: "Soft Reset",
            description: "저장만 취소하고 파일은 담긴 상태로 유지",
            icon: "📝",
            details: [
                "저장 기록만 되돌림",
                "파일은 '담긴 상태'로 유지",
        ...
        hard: {
            title: "Hard Reset",
            description: "저장과 파일을 모두 삭제 (되돌릴 수 없음)",
            icon: "⚠️",
            details: [
                "저장 기록 완전히 삭제",
                "변경된 파일도 모두 삭제",
                "작업 디렉토리를 해당 저장으로 완전히 되돌림",
                "🚨 복구 불가능 - 신중히 선택하세요"
            ],
            danger: true
        }
                ...
                <div className="modal-body">
                    <div className="reset-commit-info">
                        <div className="commit-hash">
                            <strong>저장:</strong> <code>{commitHash.slice(0, 7)}</code>
                        </div>
